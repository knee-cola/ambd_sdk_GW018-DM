...
echo ""
echo -e "${CYAN}${ICON_BUILD}\e[4mFirmware builder & Flasher\e[0m${NC}"
echo ""
...
if [[ "$PWD" != "/workspace" ]]; then
    ...
    echo ""
    echo -e "${BLUE}${ICON_INFO}To use it run:${YELLOW} ./run.sh build ${DARK_YELLOW}[--flash [device]] [--no-flash] [--no-clean]${NC}"
    echo -e ""
    exit 1
fi
